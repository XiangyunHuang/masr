...
```{r hrbrthemes, fig.cap="调用 hrbrthemes 包设置字体主题", fig.width=6, fig.height=5, out.width="75%"}
library(hrbrthemes)
...
```{r arial-narrow, fig.cap="默认字体 Arial Narrow", fig.width=6, fig.height=5, out.width="75%"}
ggplot(mtcars, aes(mpg, wt)) +
  geom_point() +
  labs(
    x = "Fuel efficiency (mpg)", y = "Weight (tons)",
...
```{r xkcd-graph, fig.cap = "漫画风格的字体方案", dev = if (knitr::is_html_output()) "svg" else if (knitr::is_latex_output()) "cairo_pdf" else "png", out.width="75%", fig.width=5, fig.height=4}
library(extrafont)
library(xkcd)
...
```{r calendar-masr, fig.cap="《现代统计图形》的活跃情况", fig.width=8, fig.height=4}
# 日历图
