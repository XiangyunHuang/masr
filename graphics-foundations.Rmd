...
```{r,fig.cap="直方图",fig.asp=1,fig.width=5.5,out.width="55%"}
set.seed(1234)
n <- 2^24
x <- runif(n, 0, 1)
delta <- 0.01
...
```{r eruptions,fig.cap="老忠实泉间歇性喷水的时间间隔分布"}
with(data = faithful, {
...
```{r,fig.asp=1,out.width="65%",fig.width=6.5}
...
```{r,fig.asp=1,out.width="55%",fig.width=5.5}
...
```{r,fig.asp=1,out.width="75%",fig.width=7.5}
library(survival)
surv.days <- Surv(days)
...
```{r faithful-ecdf,fig.cap="累积经验分布图"}
with(data = faithful, {
...
```{r,fig.cap="一维散点图",fig.subcap=c("抖动图","疏密图"),fig.asp=0.3}
...
```{r,fig.cap="地形图之海拔高度", fig.width=4, fig.height=4}
plot(y ~ x,
...
```{r category-base, fig.cap="分类散点图", out.width="75%", fig.width=4, fig.height=4}
plot(mpg ~ hp,
...
```{r iris-scatter, fig.cap="分类散点图", out.width="75%", fig.width=4, fig.height=4}
...
```{r scatter-iris, fig.cap="高亮图中部分散点", fig.subcap=c("原图", "高亮"), out.width="45%", fig.ncol=2, fig.height=4, fig.width=4}
data("iris")
...
```{r densCols,fig.cap="根据点的密度生成颜色", out.width="75%", fig.width=4.5, fig.height=4.5}
plot(faithful,
...
```{r,fig.cap="带标准差的均值散点图",fig.asp=.8,fig.width=5}
...
```{r bessel-function, dev='tikz', fig.cap="贝塞尔函数", fig.show='hold', out.width="65%", fig.width=4, fig.height=4, cache=TRUE, fig.process=to_png}
x0 <- 2^(-20:10)
nus <- c(0:5, 10, 20)
...
```{r, fig.width=4, fig.height=4, fig.cap="变量关系", out.width="75%"}
pairs(longley,
  gap = 0,
...
```{r,fig.cap="矩阵图",fig.width=6, fig.height=6, out.width="75%"}
# 自带 layout
...
```{r, fig.cap="image 图形",fig.width=4, fig.height=4, out.width="75%"}
x <- 10 * (1:nrow(volcano))
y <- 10 * (1:ncol(volcano))
...
```{r lattice-settings, fig.width=8, fig.height=8, out.width="100%"}
show.settings()
```
...
```{r smoothness,fig.width=4,fig.height=4,out.width="75%"}
set.seed(1)
...
```{r volcano-topo, fig.cap="(ref:volcano-topo)", fig.width=5.5, fig.height=5}
library(shape)
persp(volcano, 
  theta = 30, phi = 20, 
...
```{r volcano-shade, fig.cap="(ref:volcano-topo)", fig.width=5, fig.height=5, out.width="75%"}
wireframe(volcano,
  shade = TRUE,
  xlab = expression(x[1]),
  ylab = expression(x[2]),
...
```{r map, fig.width=8, fig.height=8, out.width="100%", fig.cap="(ref:county-map)"}
library(maps)
library(mapproj)
library(latticeExtra)
