...
plot(x = density(rnorm(1000)),
  type = "l",
  xlab = "$x$", ylab = "$f(x)$",
  main = "$f(x) = \\frac{1}{\\sqrt{2\\pi}}e^{-\\frac{x^2}{2}}$"
)
for (i in 1:10) {
...
```{r faithful-dens, fig.cap="散点图：faithful 数据集", fig.width=5, fig.height=5, echo=FALSE}
plot(faithful,
...
ggplot(data.frame(x = c(-4, 10)), aes(x)) +
  ...
  geom_vline(xintercept = 4, col = "gray40", lty = 2, lwd = 1) +
  geom_vline(xintercept = 2, col = "gray40", lty = 2, lwd = 1) +
  theme_void()
```
...
书籍在 `r R.version.string` 下编译，Pandoc 版本 2.14.1，最新一次编译发生在 `r format(Sys.time(), tz = "Asia/Shanghai")`。
